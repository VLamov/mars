...
class DeviceAdmin(admin.ModelAdmin):
    # на случай работы с запчастями можно добавить и  поиск по id
    search_field = ('manufacturer', 'model')  # поля по которым будет осуществляться поиск
...
class OrderAdmin(admin.ModelAdmin):
    ...
    # задаём методы для получения полей из foreign-foreign таблиц
    def my_customer(self, obj):
    ...
    # поля для поиска
    search_field = ('device__customer__customer_name', 'device__id', 'device__serial_number',
